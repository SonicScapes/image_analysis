...
`scene.json`:
    ...
    audio     <- scenes/audio/          (our recordings first, downloads second)
...
Scenes live in category subfolders (scenes/360pano/<id>, scenes/photos/<id>) rather than
flat under the scenes root, so this discovers them by walking for scene.json rather than
listing the root directly. The audio pool sits two levels up from every scene dir either
way (scenes/<category>/<id>/../../audio), so the `../../audio/<file>` paths this writes
into scene.json need no change for the nesting.
"""
...
DEFAULT_AUDIO = REPO / "scenes" / "audio"
...
def scene_dirs(root):
    """Every scene folder under the root, wherever its category subfolder put it."""
    return sorted({p.parent for p in root.rglob("scene.segmented.json")
...
def main():
    ...
    if args.scene:
        dirs = [d for d in scene_dirs(root) if d.name == args.scene]
        if not dirs:
            raise SystemExit(f"no scene named {args.scene!r} under {root}")
    else:
        dirs = scene_dirs(root)
    gaps_all = {}
